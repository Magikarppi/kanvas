...
import {
    UserRequest,
    validateCardsPostRequest,
    validateCardsPutRequest,
} from "../middleware/middleware";
import {
    allCardResponsiblePersonsDAO,
    addCardResponsiblePersonDAO,
    deleteCardResponsiblePersonDAO,
    cardResponsiblePersonDataDAO,
} from "../../database/DAOs/cardResponsibleDAO";
...
cards.get(
    ...
    async (req: UserRequest, res: Response) => {
        ...
        try {
            ...
            if (card) {
                const cardResponsiblePersons =
                    await allCardResponsiblePersonsDAO(cardId);
    ...
    async (req: UserRequest, res: Response) => {
        ...
        if (isBodyPropertiesMissing) {
            res.status(HTTP_RESPONSE_CODES.BAD_REQUEST).send(
                RESPONSE_MESSAGES.INVALID_REQ_BODY
            );
            return;
        }
        try {
        ...
        try {
            ...
            if (!found) {
                ...
                return res
                    ...
                    .send("Card responsible person deleted");
            }
        } catch (error) {
            console.error(error);
            return res
